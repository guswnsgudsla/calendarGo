...
<div class="pop-contants">
	...
	<div class="control-wrap">
		...
		<input type="hidden" id="srchPopMenuTypeCd" name="srchPopMenuTypeCd" value="<c:out value="${srchPopAuthInfo.paramPopMenuTypeCd}"/>">
			...
			<div class="filter">
				<div class="row">
					<div class="fm-ele" data-size="w600">
						<input type="text" id="srchPopMenuString" name="srchPopMenuString" value="<c:out value="${srchPopAuthInfo.paramPopMenuString}"/>" placeholder="메뉴명을 입력하세요." />
					</div>
				</div>
			</div>
			<div class="btn-wrap">
	...
	<div class="table-wrap">
		<table>
			...
			<tbody>
		<c:if test="${!empty popAuthrtRelChgHstryList}">
			<c:forEach var="popAuthrtRelChgData" items="${popAuthrtRelChgHstryList}" varStatus="status">
				<tr>
					<td><c:out value="${popAuthrtRelChgData.listChgHstryNo}"/></td>
					<td><c:out value="${popAuthrtRelChgData.chgHstryTypeNm}"/></td>
					<td class="tl pl10"><c:out value="${popAuthrtRelChgData.menuNm}"/></td>
					<td><c:out value="${popAuthrtRelChgData.menuInqAuthrtYn}"/></td>
					<td><c:out value="${popAuthrtRelChgData.prslAuthrtYn}"/></td>
					<td><c:out value="${popAuthrtRelChgData.wrtAuthrtYn}"/></td>
					<td><c:out value="${popAuthrtRelChgData.mdfcnAuthrtYn}"/></td>
					<td><c:out value="${popAuthrtRelChgData.delAuthrtYn}"/></td>
					<td><c:out value="${popAuthrtRelChgData.rgtrNcm}"/>(<c:out value="${popAuthrtRelChgData.rgtrId}"/>)</td>
					<td><c:out value="${popAuthrtRelChgData.regDtV19}"/></td>
				</tr>
			</c:forEach>
		</c:if>
		<c:if test="${empty popAuthrtRelChgHstryList}">
