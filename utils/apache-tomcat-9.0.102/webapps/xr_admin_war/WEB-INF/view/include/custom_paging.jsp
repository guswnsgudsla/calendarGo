...
<c:if test="${lastPage > 0 }">
	<div class="pagination">
		<c:if test="${block > 1 }">
			<span><a href="#!" onclick="<c:out value="${param.funNm }"/>('<c:out value="${param.url}"/>', {'currentPage' : '1'});">처음</a></span>
	...
	<c:if test="${block < totalBlock }">
		...
		<span ><a href="#!" onclick="<c:out value="${param.funNm }"/>('<c:out value="${param.url}"/>', {'currentPage' : '<c:out value="${totalPage}"/>'});">끝</a></span>
	</c:if>
	</div>
</c:if>