...
<c:if test="${lastPage > 0 }">
	...
	<c:if test="${block > 1 }">
		<span class="navi first"><a href="#!" onclick="${fn:escapeXml(param.funNm)}('${fn:escapeXml(param.url)}', {'currentPage' : '1'});">처음</a></span>
		<span class="navi prev"><a href="#!" onclick="${fn:escapeXml(param.funNm)}('${fn:escapeXml(param.url)}', {'currentPage' : '<c:out value="${currentPage - 1}"/>'});">이전</a></span>
	</c:if>
	<c:forEach var="nowPage" begin="${firstPage+1 }" end="${lastPage }" step="1">
		...
		<span class="">
			...
			<a href="#!" onclick="${fn:escapeXml(param.funNm)}('${fn:escapeXml(param.url)}', {'currentPage' : '<c:out value="${nowPage}"/>'});" ><c:out value="${nowPage}"/></a>
		</span>
	</c:forEach>
	<c:if test="${block < totalBlock }">
		<span class="navi next"><a href="#!" onclick="${fn:escapeXml(param.funNm)}('${fn:escapeXml(param.url)}', {'currentPage' : '<c:out value="${lastPage + 1}"/>'});">다음</a></span>	
		<span class="navi last"><a href="#!" onclick="${fn:escapeXml(param.funNm)}('${fn:escapeXml(param.url)}', {'currentPage' : '<c:out value="${totalPage}"/>'});">마지막</a></span>	
	</c:if>
	  <!-- pagination 모바일용 
	  <button
	    type="button"
