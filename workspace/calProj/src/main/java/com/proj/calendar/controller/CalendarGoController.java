...
public class CalendarGoController {
	...
	public String calendarList(
		...
		// 년도 for 공휴일추가
		int iYear = 2020;
		CalendarGoVO selAnvYnList = calendarGoService.selectAnvYnList(calendarGoVO);
		List<CalendarGoVO> yearList = new ArrayList<CalendarGoVO>();
		for (int i = 0; i <= 10; i++) {
...
//		System.out.println("=======");
		...
		CalendarGoVO selAnvYn = calendarGoService.selectAnvYn(calendarGoVO);
		if (selAnvYn == null) {
...
//			System.out.println("=======");
			...
			if (ok) {
				...
				for (int i = 0; i < ns.getLength(); i++) {
					...
					calendarGoVO.setCalAnvYn("Y");
					calendarGoVO.setCalAnvYear(calendarGoVO.getYearData());
					calendarGoVO.setVCont(e.getElementsByTagName("dateName").item(0).getTextContent());
					calendarGoVO.setVStrDt(e.getElementsByTagName("locdate").item(0).getTextContent());
					calendarGoVO.setVEndDt(e.getElementsByTagName("locdate").item(0).getTextContent());
					
					calendarGoService.insertCalendarGo(calendarGoVO);
					cnt++;
				}
			}
	...
	@PostMapping("/calendarAnivDel.do")
	@ResponseBody
	public String calendarAnivDel(
			@ModelAttribute("calVO") CalendarGoVO calendarGoVO, 
			HttpServletRequest request,
			ModelMap model
	) throws Exception {
...
//		System.out.println("=======");
		...
		// 성공YN
		int cnt = calendarGoService.deleteAnvYn(calendarGoVO);
		if (cnt > 0) {
			return "success";
		} else {
			return "fail";
