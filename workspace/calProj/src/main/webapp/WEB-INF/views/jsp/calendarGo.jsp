	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		var calendar = new FullCalendar.Calendar(calendarEl, {
			...
			eventClick: function(info) { //일정 선택 시 기능
				//공휴일인지아닌지
				var vAnivYn = info.event.extendedProps.description;
			...
			select: function(info) { //빈칸 선택 시 기능
				...
				$('#shareCalendar').css('display', 'contents');
			...
			customButtons: { //일정추가버튼 기능
				...
				addAniversButton: {
					text: '공휴일추가',
					click: function() {
						$('#selAnivers option:eq(0)').prop("selected", true);
		...
		$('#addAnivers').click( function(){
			...
			var selYn  = $("#selAnivers>option:selected").attr('data-yn');
			
			if (selYn == 'Y') {
				alert("이미 추가되어있는 공휴일 년도입니다.");
				return false;
			}
		...
		$('#delAnivers').click( function(){
			...
			var answer = confirm(selVal+"년도의 공휴일을 삭제하시겠습니까?");
			if (answer) {
				$.ajax({
					type: "POST",
					url: "/calendarAnivDel.do",
					data: {
						yearData: selVal
					},
					success: function(returnData) {
						if (returnData == 'success') {
							alert("해당년도 공휴일이 삭제되었습니다.");
							window.location.reload();
						}else {
							alert("공휴일삭제에 실패하였습니다. 재시도하여주시기 바랍니다.");
						}
					}
				});
			}else {
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">
						...
						<select class="form-control" id="selAnivers" name="selAnivers" aria-label="Default select example">
							<c:forEach var="result" items="${yearList}">
							<option value="${result.yearList}" data-yn="${result.yearYn }">${result.yearList}<c:if test="${result.yearYn eq 'Y' }"><span> (추가완료)</span></c:if></option>
							</c:forEach>
						</select>
					</div>
				</div>
